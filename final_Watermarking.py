...
# Print the generated watermark and its encoded form
print("Generated Watermark:", watermark)
print("First 50 bits of the encoded watermark:", encoded_watermark[:50])
...
# Embed and Extract Watermark for Testing Parameters
...
# Final Embedding with Optimal Parameters
magnitude_copy = magnitude.copy()
for i in range(magnitude.shape[1]):
    magnitude_copy[:, i] += best_alpha * encoded_watermark[i % len(encoded_watermark)] * np.max(magnitude_copy[:, i])

watermarked_D = magnitude_copy * np.exp(1j * phase)
watermarked_audio = librosa.istft(watermarked_D)

# Print the generated watermark and the first 50 bits of the encoded watermark
print("Generated Watermark:", watermark)
print("First 50 bits of the encoded watermark:", encoded_watermark[:50])

# Save the final watermarked audio
sf.write('E:/chaanged/watermarked_audio_with_ecc.wav', watermarked_audio, sr)
print("\nFinal embedding completed with optimal parameters.")
